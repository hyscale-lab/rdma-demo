...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("x-amz-id-2", "s3-rdma-server")
...
func (h *Handler) handleObject(w http.ResponseWriter, r *http.Request, reqID, bucket, key string) {
	switch r.Method {
	case http.MethodPut:
		result, err := consumeBody(r.Body, h.maxObjectSize)
		if err != nil {
...
func consumeBody(body io.ReadCloser, maxObjectSize int64) (consumeResult, error) {
	defer body.Close()
	...
	for {
		...
		if n > 0 {
			...
			if maxObjectSize > 0 && size > maxObjectSize {
				return consumeResult{}, fmt.Errorf("object too large: %d > %d", size, maxObjectSize)
			}
			if _, writeErr := hasher.Write(buf[:n]); writeErr != nil {
				return consumeResult{}, writeErr
			}
		}
		if err == io.EOF {
			break
	...
	return consumeResult{
		ETag: quotedHash(hasher),
		Size: size,
	}, nil
}

func quotedHash(hasher hash.Hash) string {
