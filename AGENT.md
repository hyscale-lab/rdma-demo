...
- After any user correction or a discovered mistake:
  - add a new entry to `agent/tasks/lessons.md` capturing:
    - the failure mode, the detection signal, and a prevention rule.
- Review `agent/tasks/lessons.md` at session start and before major refactors.
...
1. **Plan First**
   - Write a checklist to `agent/tasks/todo.md` for any non-trivial work.
...
6. **Capture Lessons**
   - Update `agent/tasks/lessons.md` after corrections or postmortems.
...
- Maintain a short running "Working Notes" section in `agent/tasks/todo.md`:
...
### Plan Template (Paste into `agent/tasks/todo.md`)
